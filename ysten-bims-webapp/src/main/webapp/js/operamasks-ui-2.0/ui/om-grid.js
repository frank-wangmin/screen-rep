    ...
    $.omWidget('om.omGrid', {
        options:/** @lends omGrid#*/{
            ...
            */
            ...
            /**
             * extraDataSource = [{name:"fieldName", url : "some_object.json"},{name:"fieldName", url : "some_object.json"}]
             */
        ...
        _init:function(){
            ...
            if(this.options.extraDataSource){
            	this._proccessExtraDataSource(this.options.extraDataSource, 0);
            } else {
            	this._populate();
            }
        },
        _proccessExtraDataSource : function(extraDataSource, level){
        	 var self=this,
	             el = this.element,
	             grid = el.closest('.om-grid'),
	             op = this.options,
	             pageStat = $('.pPageStat', grid);
        	 var loadMask = $('.gBlock',grid);
	         //具备加载数据的前提条件了，准备加载
	         pageStat.html($.om.lang._get(op,"omGrid","loadingMsg"));
	         loadMask.show();
        	$.ajax({
   			 	type : op.method,
                url : extraDataSource[level].url,
                dataType : 'json',
                success : function(data){
                	var name = extraDataSource[level].name;
               	 	op[name] = data;
               	 	if(level < extraDataSource.length - 1){
               	 		self._proccessExtraDataSource(extraDataSource, level+1);
               	 	} else if(level === extraDataSource.length - 1){
               	 		self._populate();
               	 	}
                },
                ...
                bodyContent.push("<tr _grid_row_id="+(self._guid++)+" class='om-grid-row " + rowCls + "'>");
               	...
               	$(gridHeaderCols).each(function(index){
                    ...
                    if(axis == 'indexCol'){
                        ...
                        cls = colModel[colIndex].cls ? colModel[colIndex].cls : "";
                    ...
                    cols = [this.align , this.abbr , axis , cls, this.align , wrap?'wrap':'', headerWidth[index] , html];
                    j=0;
        ...
        _buildRowCellValues:function(colModel,rowData,rowIndex){
            ...
            for(var i=0;i<len;i++){
                ...
                if(typeof r === 'function'){
                    v=r(v,rowData,rowIndex, this.options);
                }
                values[i]=v;
                v = null;
            }
