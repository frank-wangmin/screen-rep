...
;(function($) {
	...
	$.omWidget('om.omCombo', {
        ...
        _init:function(){
            ...
            //if (options.multi) {
			// 强制所有下拉框可输入并自动可过滤。忽略外面的不可编辑选项。李雪君
                options.editable = this.options.editable = true;
				options.autoFilter = this.options.autoFilter = true;
				options.forceSelection = this.options.forceSelection = true;
			    options.filterStrategy = this. options.filterStrategy = "anywhere";
            //}
            ...
            input.focus(function(){
                ...
                if (!options.disabled && !options.readOnly && !options.multi) {
                    ...
                    self._refeshEmptyText(emptyText);
                	 ...
                	 var value = '';
                	 var text = '';
                     var selectedValues = input.val().split(options.multiSeparator);
                     for (var i=0; i<selectedValues.length; i++) {
                         var index = allInputText.indexOf(selectedValues[i]);
                         if (index > -1) {
                        	 if (text.length > 0) {
                        		 text += ',';
                        	 }
        ...
        _doFilter:function(){
            ...
            if (options.multi) {
			    var index = text.lastIndexOf(options.multiSeparator);
				if (index != -1) {
